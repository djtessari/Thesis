...
void *LoopPlate(void *threadarg){
	...
	for (i = 0; i < iterations; i++) {
		...
		sem_post(&done);
		sem_getvalue(&waiting, &sem_val1);
		sem_getvalue(&done, &sem_val2);

		if (sem_val1 == 0 && sem_val2 == num_threads)
		{
			printf("Unlock all threads\n");
		...
		{
			printf("Thread %i finished loop.  Waiting\n", id);
			pthread_mutex_unlock(&sem_mutex);
	...
	printf("Exit loop, closing thread\n");
...
int main(int argc, char **argv) {
	...
	sem_init(&waiting, 0, num_threads);
	sem_init(&done, 0, 0);
